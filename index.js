...
function startRecursive() {
  return new Promise(resolve => {
    if (users.length > 0) {
        recursive();
        function recursive() {
          let fetchUsers = users.splice(0, 10);
          let promises = [];
          for (let i = 0; i < fetchUsers.length; i++) {
            promises.push(processUser(fetchUsers[i].id));
          }
          Promise.all(promises)
            .then(values => {
              let tenVal = values.reduce((acc, val) => acc + val);
              total.push(tenVal);
            })
            .then(() => {
              if (users.length > 0) {
                ...
                let result = total.reduce((acc, val) => acc + val)
                resolve(result);
              }
            });
        }
